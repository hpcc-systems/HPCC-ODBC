...
HPCCdb::HPCCdb(TM_ModuleCB tmHandle, const char * _protocol, const char * _wsSqlPort, const char * _user, const char * _pwd, const char * _espIP, const char * _targetCluster, const char * _defaultQuerySet, aindex_t _maxFetchRowCount)
{
    ...
    tm_trace(driver_tm_Hdle, UL_TM_INFO, "HPCC_Conn: protocol='%s', wsSqlPort='%s', user='%s', espIP='%s', targetCluster='%s', defaultQuerySet='%s', maxFetchRowCount='%d'\n", (_protocol,_wsSqlPort,_user,_espIP,_targetCluster,_defaultQuerySet,_maxFetchRowCount));

    //Create connection object
    m_clientWs_sql.setown(createwssqlClient());
    StringBuffer esp;
    esp.appendf("%s://%s:%s/wssql", m_protocol, m_sESP, m_wsSqlPort);
    m_clientWs_sql->addServiceUrl(esp.str());
...
bool HPCCdb::ping()
{
    ...
    tm_trace(driver_tm_Hdle, UL_TM_INFO, "HPCC_Conn:call to HPCCdb::ping\n", (0));

    Owned<IClientEchoRequest> req;
    Owned<IClientEchoResponse> resp;

    req.setown( createClientEchoRequest());
    const char * msg = "Hello HPCC!";
    req->setRequest(msg);
    try
    {
        tm_trace(driver_tm_Hdle, UL_TM_INFO, "HPCC_Conn:calling ws_sql.Echo('%s')...\n", (msg));
        resp.setown(m_clientWs_sql->Echo(req));//calls ws_sql
    ...
    return (resp && 0 == sl_strcmp(resp->getResponse(),msg));
}
